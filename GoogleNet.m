...
%%
mkdir('GoogleNetDatabase');   %main folder
mkdir('GoogleNetDatabase\NORMAL'); %sub folder
mkdir('GoogleNetDatabase\AF');
% mkdir('GoogleNetDatabase\IAVB');
mkdir('GoogleNetDatabase\LBBB');
mkdir('GoogleNetDatabase\RBBB');
% mkdir('GoogleNetDatabase\PAC');
% mkdir('GoogleNetDatabase\PVC');
...
for ii = 1:size(NORMAL,1)
    ...
    imageRoot = 'C:\Users\Tuf\Desktop\Dataset Licenta\GoogleNetDatabase\NORMAL\';
    saveTimeFrequencyFigure(abs(wt), ii, 'NORMAL',imageRoot)
end

for ii = 1:size(AF,1)
    [wt, f] = fb.wt(AF(ii, :));
    imageRoot = 'C:\Users\Tuf\Desktop\Dataset Licenta\GoogleNetDatabase\AF\';
    saveTimeFrequencyFigure(abs(wt), ii, 'AF',imageRoot)
end

% for ii = 1:size(IAVB,1)
%     [wt, f] = fb.wt(IAVB(ii, :));
%     imageRoot = 'C:\Users\Tuf\Desktop\Dataset Licenta\GoogleNetDatabase\IAVB\';
%     saveTimeFrequencyFigure(abs(wt), ii, 'IAVB',imageRoot)
% end

for ii = 1:size(LBBB,1)
    [wt, f] = fb.wt(LBBB(ii, :));
    imageRoot = 'C:\Users\Tuf\Desktop\Dataset Licenta\GoogleNetDatabase\LBBB\';
    saveTimeFrequencyFigure(abs(wt), ii, 'LBBB',imageRoot)
end

for ii = 1:size(RBBB,1)
    [wt, f] = fb.wt(RBBB(ii, :));
    imageRoot = 'C:\Users\Tuf\Desktop\Dataset Licenta\GoogleNetDatabase\RBBB\';
    saveTimeFrequencyFigure(abs(wt), ii, 'RBBB',imageRoot)
end

% for ii = 1:size(PAC,1)
%     [wt, f] = fb.wt(PAC(ii, :));
%     imageRoot = 'C:\Users\Tuf\Desktop\Dataset Licenta\GoogleNetDatabase\PAC\';
%     saveTimeFrequencyFigure(abs(wt), ii, 'PAC',imageRoot)
% end
% 
% for ii = 1:size(PVC,1)
%     [wt, f] = fb.wt(PVC(ii, :));
%     imageRoot = 'C:\Users\Tuf\Desktop\Dataset Licenta\GoogleNetDatabase\PVC\';
...
title("Confusion Matrix GoogleNet, Accuracy " + round(accuracy*100,1) + "%");
xlabel("Predictie");
ylabel("Adevar");