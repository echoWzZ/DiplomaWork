...
NORMAL_tot = data_3000(r1,:);     %Taken all recordings label 1 - 7
AF_tot = data_3000(r2,:);
...
% end
%%
mkdir('AlexNetDatabase');   %main folder
mkdir('AlexNetDatabase\NORMAL'); %sub folder
mkdir('AlexNetDatabase\AF');
% mkdir('AlexNetDatabase\IAVB');
mkdir('AlexNetDatabase\LBBB');
mkdir('AlexNetDatabase\RBBB');
% mkdir('AlexNetDatabase\PAC');
% mkdir('AlexNetDatabase\PVC');
...
for ii = 1:size(NORMAL,1)
    ...
    imageRoot = 'C:\Users\Tuf\Desktop\Dataset Licenta\AlexNetDatabase\NORMAL\';
    saveTimeFrequencyFigure(abs(wt), ii, 'NORMAL',imageRoot)
end

for ii = 1:size(AF,1)
    [wt, f] = fb.wt(AF(ii, :));
    imageRoot = 'C:\Users\Tuf\Desktop\Dataset Licenta\AlexNetDatabase\AF\';
    saveTimeFrequencyFigure(abs(wt), ii, 'AF',imageRoot)
end

% for ii = 1:size(IAVB,1)
%     [wt, f] = fb.wt(IAVB(ii, :));
%     imageRoot = 'C:\Users\Tuf\Desktop\Dataset Licenta\AlexNetDatabase\IAVB\';
%     saveTimeFrequencyFigure(abs(wt), ii, 'IAVB',imageRoot)
% end

for ii = 1:size(LBBB,1)
    [wt, f] = fb.wt(LBBB(ii, :));
    imageRoot = 'C:\Users\Tuf\Desktop\Dataset Licenta\AlexNetDatabase\LBBB\';
    saveTimeFrequencyFigure(abs(wt), ii, 'LBBB',imageRoot)
end

for ii = 1:size(RBBB,1)
    [wt, f] = fb.wt(RBBB(ii, :));
    imageRoot = 'C:\Users\Tuf\Desktop\Dataset Licenta\AlexNetDatabase\RBBB\';
    saveTimeFrequencyFigure(abs(wt), ii, 'RBBB',imageRoot)
end

% for ii = 1:size(PAC,1)
%     [wt, f] = fb.wt(PAC(ii, :));
%     imageRoot = 'C:\Users\Tuf\Desktop\Dataset Licenta\AlexNetDatabase\PAC\';
%     saveTimeFrequencyFigure(abs(wt), ii, 'PAC',imageRoot)
% end
% 
% for ii = 1:size(PVC,1)
%     [wt, f] = fb.wt(PVC(ii, :));
%     imageRoot = 'C:\Users\Tuf\Desktop\Dataset Licenta\AlexNetDatabase\PVC\';
%     saveTimeFrequencyFigure(abs(wt), ii, 'PVC',imageRoot)
% end

%%
% readFcn = @(imagefilename)imresize(imread(imagefilename), [227 227]);
% filepath = "AlexNetDatabase";
% Dataset = imageDatastore(filepath, "IncludeSubfolders", true, 'LabelSource', 'foldernames', 'ReadFcn', readFcn);
filepath1 = "Training";
...
net = alexnet;
% layers = alex.Layers;
% layers(23) = fullyConnectedLayer(9,'WeightLearnRateFactor',10,'BiasLearnRateFactor',10);
% layers(24) = softmaxLayer();
% layers(25) = classificationLayer("Classes", {'AF','IAVB','LBBB', 'NORMAL','PAC','PVC','RBBB','STD','STE'});
% 
...
Feature_Learner = net.Layers(23).Name;
Output_Classifier = net.Layers(25).Name;
...
'MaxEpochs', 12,'MiniBatchSize', MiniBatchSize, 'Plots','training-progress','ExecutionEnvironment',"gpu",'Shuffle','every-epoch','LearnRateSchedule', "piecewise", "ValidationFrequency", Validation_Frequency, "LearnRateDropFactor",0.2, "LearnRateDropPeriod",5);
...
title("Confusion Matrix AlexNet, Accuracy " + round(accuracy*100, 1) + "%");
xlabel("Predictie");
ylabel("Adevar");